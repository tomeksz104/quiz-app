    ...
    <div class="container flex-col relative overflow-y-auto">
        <div class="px-4 md:px-10 mt-5">
            <div class="flex justify-between px-3 items-center">
                ...
                <button wire:click='$emit("openModal", "admin.modals.category-create-update", {{ json_encode(["updateMode" => true]) }})'
            ...
            <div x-data="{ activeBulkDelete: @entangle('activeBulkDelete') }"  class="bg-white my-10 shadow-2xl rounded-2xl ">
                <div class="flex justify-between px-4 py-3">
                    ...
                    <div class="flex justify-center items-center relative xl:w-96">
                        ...
                        <input wire:model="search" type="search" aria-label="Filter projects" placeholder="Filtruj kategorie..." class="appearance-none w-full text-sm leading-6 bg-transparent text-slate-900 placeholder:text-slate-400 rounded-md py-2 pl-10 border-gray-300 shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
                    </div>
                </div>
                <table class="items-center w-full bg-white border-collapse">
                    <thead>
                        <tr class="border border-x-0 border-blue-100 bg-blue-50 text-slate-500 text-base uppercase whitespace-nowrap text-left font-semibold">
                            ...
                            <th class="px-6 py-3">Wyświetlenia</th>
                    ...
                    @forelse ($categories as $category)
                        <tr class="text-slate-500 hover:bg-slate-50">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <input wire:click="addSelectedCategory({{ $category->id }})" id="selectedCategories" type="checkbox" class=" appearance-none h-4 w-4 border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" @if(in_array($category->id, $selectedCategories)) checked @endif >
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <img src="{{ isset($category->image->path) ? asset($category->image->path) : asset("uploads/images/no-image.jpg") }}" class="w-24 rounded-xl"/>
                                    <span class="flex justify-start ml-3 font-bold text-gray">
                                        {{ $category->title }}
                                    </span>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex justify-end">
                                    <button wire:click='$emit("openModal", "admin.modals.category-create-update", {{ json_encode(["updateMode" => false, "category" => $category->id]) }})' class="flex items-center text-slate-500 hover:text-slate-400 mr-3"  @popper(Podgląd)>
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                        </svg>
                                    </button>
                                    <button wire:click='$emit("openModal", "admin.modals.category-create-update", {{ json_encode(["updateMode" => true, "category" => $category->id]) }})' class="flex items-center text-blue-500 hover:text-blue-400 mr-3" @popper(Edycja)>
                                    ...
                                    <button type="button"
                                            wire:click="deleteConfrim({{ $category->id }})"
